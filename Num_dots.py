...
лэйбл-массив, присваивая каждому элементу массива каждого объекта определенное значение - лэйбл объекта. Переменной num_dots присваивается значение
количества объектов.
Далее, с помощью лэйбл-массива рассчитывается количество элементов массива в каждом объекте изображения (Counter(labels)), и определяется количество элементов
как в наименьшем, так и в наибольшем объекте. При этом количество нулей (количество элементов фона) в объекте Counter занижается до нуля, чтобы не влиять на
дальнейшие расчеты.
После этого устанавливаются 3 лимита для количества элементов массива в каждом объекте изображения, позволяющие разделить объекты на 4 группы, как
состоящие из 1-й, 2-х, 3-х или 4-х точек. При этом лимиты устанавливаются следующим образом: limit1 - количество элементов в наименьшем объекте плюс 10%,
limit2 = limit1 + int((maxlabels_counter - limit1)*0.33), limit3 = limit1 + int((maxlabels_counter - limit1)*0.66), где maxlabels_counter - количество
элементов массива изображения в наибольшем объекте.
В заключении запускается цикл, сортирующий объекты изображения по 3-м группам (наименьшие объекты из первой группы не учитываются), при этом, при попаданиии
объекта в группу между limit1 и limit2 значение num_dots увеличивается на 1, при попаданиии объекта в группу между limit2 и limit3 значение num_dots
увеличивается на 2, а при попадании объекта в группу выше значение num_dots значение num_dots увеличивается на 3.
